...
def Objects_combineall16bitunsignedshort(f):
    lenSize=0
    ...
    for i, obdata in enumerate(bpy.data.meshes):
        ...
        f.write(pack("B", 192))
        f.write(pack("B", len(obdata.vertices)))
        f.write(pack("B", 109)) #0x6D
        for v in obdata.vertices:
            f.write(pack("<H", int(4096*color_layer[v.index].color[0])))
            f.write(pack("<H", int(4096*color_layer[v.index].color[1])))
            f.write(pack("<H", int(4096*color_layer[v.index].color[2])))
            f.write(pack("<H", int(4096*color_layer[v.index].color[3])))
