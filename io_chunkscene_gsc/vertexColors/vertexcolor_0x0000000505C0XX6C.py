...
def Objects_combineallFloatThree(f):
    lenSize=0
    ...
    for i, obdata in enumerate(bpy.data.meshes):
        ...
        f.write(pack("B", 192))
        f.write(pack("B", len(obdata.vertices)))
        f.write(pack("B", 108)) #0x6C
        for v in obdata.vertices:
            f.write(pack("<f", color_layer[v.index].color[0]))
            f.write(pack("<f", color_layer[v.index].color[1]))
            f.write(pack("<f", color_layer[v.index].color[2]))
            f.write(pack("<f", color_layer[v.index].color[3]))
